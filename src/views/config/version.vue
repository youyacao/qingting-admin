  ...
  <div class="app-container">
    <el-card>
      ...
      <el-table ref="multipleTable" v-loading="loading" tooltip-effect="dark" :data="list" style="width: 100%;" row-key="id" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column align="center" label="版本名称" width="200">
          <template slot-scope="scope">
            {{ scope.row.versionName }}
          </template>
        </el-table-column>
        <el-table-column align="center" label="版本信息" width="200">
          <template slot-scope="scope">
            {{ scope.row.versionInfo }}
          </template>
        </el-table-column>
        <el-table-column align="center" label="安卓版本下载链接" width="200">
          <template slot-scope="scope">
            {{ scope.row.downloadUrl }}
          </template>
        </el-table-column>
        <el-table-column align="center" label="苹果版本下载链接" width="200">
          <template slot-scope="scope">
            {{ scope.row.downloadUrl_IOS }}
          </template>
        </el-table-column>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'编辑':'新增'">
      <el-form :model="data" label-width="140px">
        <el-form-item label="是否强制更新">
          <el-switch v-model="data.forceUpdate" active-color="#13ce66" inactive-color="#ff4949" />
        </el-form-item>
        <el-form-item label="版本号">
          <el-input v-model="data.versionCode" placeholder="版本号" />
        </el-form-item>
        <el-form-item label="版本名称">
          <el-input v-model="data.versionName" placeholder="版本名称" />
        </el-form-item>
        <el-form-item label="版本信息">
          <el-input v-model="data.versionInfo" type="textarea" :rows="2" placeholder="版本信息" />
        </el-form-item>
        ...
        <el-form-item label="苹果版本下载链接">
          <el-input v-model="data.downloadUrl_IOS" placeholder="苹果版本下载链接" />
        </el-form-item>
      </el-form>
...
import { getDatas, addData, deleteData, updateData, batchDisable } from '@/api/version'
...
export default {
  ...
  methods: {
    ...
    handleSuccess(res, file) {
      if (res.code === 200) {
        this.data.downloadUrl = res.data.url
      } else {
        this.$message({
          type: 'error',
          message: res.msg
